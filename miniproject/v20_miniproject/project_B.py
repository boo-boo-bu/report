...
def load_models():
    ...
    classifier = pipeline("zero-shot-classification", model="beomi/kcbert-base")
...
def map_voc_to_category(voc_text):
    ...
    voc_lower = voc_text.lower().replace(" ", "")
    
    if any(keyword in voc_lower for keyword in ['price', 'expensive', 'cost', '가격', '비싼', '비용', '가성비', '비싸', '저렴', '할인', '행사', '창렬', '혜자', '부담', '돈']):
        return "가격"
    elif any(keyword in voc_lower for keyword in ['delivery', 'shipping', 'time', '배송', '택배', '포장', '파손', '느려', '터져', '칼배송', '상태', '꼼꼼', '찌그러', '도착']):
        return "배송/포장"
    elif any(k in voc_lower for k in ['vomit', 'diarrhea', 'allergy', '구토', '설사', '알러지', '눈물', '부작용', '가려워', '긁어', '독해', '무서워', '위험', '이상해']):
        return "안전성"
    elif any(keyword in voc_lower for keyword in ['size', 'small', 'tablet', '알약', '크기', '작은', '부수기', '딱딱', '편의', '가루', '날림', '급여', '간편', '귀찮', '힘들어', '목넘김']):
        return "편의성"
    elif any(keyword in voc_lower for keyword in ['taste', 'bitter', 'flavor', '맛', '쓴', '냄새', '잘먹', '안먹', '거부', '기호성', '숨겨서', '섞어', '뱉어', '환장', '순삭', '입맛']):
        return "기호성"
    elif any(keyword in voc_lower for keyword in ['health', 'nutrition', 'benefit', '영양', '건강', '성분', '효과', '기능', '함량', '개선', '변화', '눈에 띄게', '그대로', '개선', '도움']):
        return "영양/건강"
    else:
        return None
    ...
    if st.button("⚖️ 카피 승자 예측"):
        if not copy_a or not copy_b:
            ...
            with st.spinner('AI가 광고 효과를 시뮬레이션 중입니다...'):
                ...
                # 2. 제로샷 분류용 레이블 설정 (구체적으로 작성해야 AI가 안 헷갈려합니다)
                labels_desc = ["가성비 및 가격", "배송 속도 및 포장", "성분 안전성 및 부작용", "급여 편의성", "맛과 기호성", "영양 성분 및 효과"]
                res_zero_a = zero_shot_pipeline(copy_a.strip(), labels_desc)
                res_zero_b = zero_shot_pipeline(copy_b.strip(), labels_desc)
                
                # AI 긴 레이블을 짧은 키워드로 변환하는 사전
                label_fix = {
                    "가성비 및 가격": "가격",
                    "배송 속도 및 포장": "배송/포장",
                    "성분 안전성 및 부작용": "안전성",
                ...
                # 3. AI 결과값 추출 및 변환 (이 부분이 핵심입니다)
                # AI가 판단한 긴 문장을 '가격', '안전성' 같은 짧은 단어로 바꿉니다.
                top_label_a = label_fix[res_zero_a['labels'][0]]
                top_score_a = round(res_zero_a['scores'][0]*100, 1)

                top_label_b = label_fix[res_zero_b['labels'][0]]
                top_score_b = round(res_zero_b['scores'][0]*100, 1)
            ...
            with c1:
                ...
                st.write(f"**AI 분석 속성:** {top_label_a} ({top_score_a}%)")
            with c2:
                st.metric("B안 (긍정도)", f"{res_b['label']}", f"{round(res_b['score']*100, 1)}%")
                st.write(f"**AI 분석 속성:** {top_label_b} ({top_score_b}%)")

            st.divider()

            # --- [핵심 로직 보정] ---

            # 5. 매핑 함수를 통한 키워드 강제 분류 (Expert Rule)
            copy_a_cat = map_voc_to_category(copy_a)
            copy_b_cat = map_voc_to_category(copy_b)
            
            # 6. 해결해야 할 불만 사항 가져오기
            neg_text = st.session_state.get('negative_summary', '')
            voc_category = map_voc_to_category(neg_text)
            if not voc_category:
                voc_category = "전반적 만족도"

            # 7. 승자 결정 (우선순위: 1.키워드 매칭 -> 2.AI 속성 매칭 -> 3.감성 점수)
            winner = None
            win_type = ""

            # 규칙 1: 키워드로 봤을 때 VOC를 해결하는 경우 (최우선)
            if copy_a_cat == voc_category and copy_b_cat == voc_category:
                winner = "A안" if top_score_a >= top_score_b else "B안"
                win_type = "VOC 해결 (키워드 매칭)"
            elif copy_a_cat == voc_category:
                winner = "A안"
                win_type = "VOC 해결 (키워드 단독)"
            elif copy_b_cat == voc_category:
                winner = "B안"
                win_type = "VOC 해결 (키워드 단독)"
            
            # 규칙 2: 키워드에는 없지만 AI가 속성을 맞게 파악한 경우
            ...
            # 규칙 3: 모두 해당 없으면 긍정도 점수 비교
            else:
                winner = "A안" if res_a['score'] >= res_b['score'] else "B안"
                win_type = "감성 점수 우세"

            # 8. 리포트 출력
            st.subheader("🏆 카피 승자 분석")
            st.success(f"🎉 **최종 승자: {winner}** ({win_type})")
            
            f_score = top_score_a if winner == "A안" else top_score_b
            f_senti = round(res_a['score']*100, 1) if winner == "A안" else round(res_b['score']*100, 1)
            
            st.info(f"**🎯 마케팅 근거 리포트**")
            st.write(f"- 분석된 고객 페인 포인트: **'{voc_category}'**")
            st.write(f"- 선정된 카피의 {voc_category} 해결 적합도: **{f_score}%**")
            st.write(f"- 소비자가 느끼는 광고 긍정 확신도: **{f_senti}%**")
